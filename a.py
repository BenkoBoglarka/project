...
def modell_felallitasa():
    ...
    #print(szokincs)
    #print('commission' in w2v.wv.key_to_index.keys())
    #Vektorok megadása
    def vektorok_kiszamitasa(mondat):
        #https://stackoverflow.com/questions/66868221/gensim-3-8-0-to-gensim-4-0-0
        # if x in list(szokincs[0])
       # return np.mean([w2v.wv[x] for x in mondat if x in list(w2v.wv.index_to_key)], axis=0).reshape(1,-1)
        return np.mean([w2v.wv[x] for x in mondat], axis=0).reshape(1,-1)
    ...
    #print(mondatok_2)
    mondatok_2[['mondat','kategoria','cimke', 'eredeti']].to_csv('kesz.csv')
    #print(mondatok_2)
    # Pontosság
